...
///
/// Returns `Some(Installable)` if the environment variable is set and can be
/// parsed, or `None` if the environment variable is not set.
...
///
/// - `config_type`: e.g. "homeConfigurations", "nixosConfigurations",
...
pub fn handle_specialisation(
  ...
  if no_specialisation {
    ...
    let current_specialisation =
      std::fs::read_to_string(specialisation_path).ok();
...
pub fn process_specialisation(
  ...
  let target_specialisation = handle_specialisation(
    specialisation_path,
    no_specialisation,
    specialisation,
  );
...
/// It unifies what would otherwise be duplicated across platform-specific
/// modules.
///
...
/// * `config_type` - The configuration type (e.g., "nixosConfigurations",
///   "darwinConfigurations")
...
/// * `skip_compare` - Whether to skip comparing the new and current
///   configuration
///
/// # Returns
///
...
pub fn get_target_hostname(
  ...
  let target_hostname = match explicit_hostname {
    ...
    None => {
      match system_hostname.clone() {
        Some(hostname) => hostname,
        None => {
          bail!(
            "Unable to fetch hostname automatically. Please specify \
             explicitly with --hostname."
          )
        },
      }
    },
...
pub fn activate_nixos_configuration(
  ...
  let switch_to_configuration =
    switch_to_configuration.canonicalize().map_err(|e| {
      color_eyre::eyre::eyre!(
        "Failed to canonicalize switch-to-configuration path: {}",
        e
      )
    })?;
...
pub struct RebuildWorkflowConfig<'a> {
  ...
  /// The configuration type (e.g., "nixosConfigurations",
  /// "darwinConfigurations")
...
/// Execute common actions for a rebuild operation across platforms using
/// configuration struct
///
/// This function takes a configuration struct instead of many individual
/// parameters
fn handle_rebuild_workflow_with_config(
  config: RebuildWorkflowConfig,
) -> Result<PathBuf> {
