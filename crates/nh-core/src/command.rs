...
impl Command {
  ...
  fn build_sudo_cmd(&self) -> Result<Exec> {
    ...
    // Support both NH_SUDOOPTS (preferred) or NIX_SUDOOPTS for nixos-rebuild
    // compatibility. Extra arguments are inserted into the sudo invocation
    // before `env`.
    if program_name == "sudo" {
      let sudoopts = env::var("NH_SUDOOPTS")
        .or_else(|_| env::var("NIX_SUDOOPTS"))
        .unwrap_or_default();
      if !sudoopts.is_empty() {
        match shlex::split(&sudoopts) {
          Some(parsed) => {
            for opt in parsed {
              cmd = cmd.arg(opt);
            }
          ...
          None => {
            warn!(
              "Failed to parse sudo opts from NH_SUDOOPTS/NIX_SUDOOPTS, \
               ignoring. Value: {sudoopts}"
            );
          },
        }
      }
  ...
  fn build_sudo_parts(&self) -> Result<Vec<String>> {
    ...
    // NH_SUDOOPTS (preferred) or NIX_SUDOOPTS (nixos-rebuild compatibility):
    // extra arguments inserted into the sudo invocation before `env`.
    if program_name == "sudo" {
      let sudoopts = env::var("NH_SUDOOPTS")
        .or_else(|_| env::var("NIX_SUDOOPTS"))
        .unwrap_or_default();
      if !sudoopts.is_empty() {
        match shlex::split(&sudoopts) {
          Some(parsed) => parts.extend(parsed),
          None => {
            warn!(
              "Failed to parse sudo opts from NH_SUDOOPTS/NIX_SUDOOPTS, \
               ignoring. Value: {sudoopts}"
  ...
  pub fn run(&self) -> Result<()> {
    ...
    let cmd = if self.elevate.is_some() && self.ssh.is_none() {
      ...
      if program_name == "sudo" {
        ...
        // NH_SUDOOPTS (preferred) or NIX_SUDOOPTS (nixos-rebuild
        // compatibility): extra arguments inserted into the sudo
        // invocation before `env`. nixos-rebuild-ng applies these in
        // both local and remote paths; match that behaviour here.
        let sudoopts = env::var("NH_SUDOOPTS")
          .or_else(|_| env::var("NIX_SUDOOPTS"))
          .unwrap_or_default();
        if !sudoopts.is_empty() {
