...
fn os_installable_at(
  ...
  for name in ["system.nix", "default.nix"] {
    let path = system_directory.join(name);
    if path.is_file() {
      debug!(path = %path.display(), "Using {name}");
      return Ok(file(path));
    }
  }
...
mod tests {
  ...
  #[test]
  fn etc_default_nix_precedes_nixpkgs_fallback() {
    let etc = tempfile::tempdir().unwrap();
    let default_nix = etc.path().join("default.nix");
    ...
    assert!(matches!(
      installable,
      Installable::File { path, attribute }
        if path == default_nix && attribute.is_empty()
    ));
  }

  #[cfg(unix)]
  #[test]
  fn resolve_flake_directory_resolves_symlinked_directory() {
    use std::os::unix::fs::symlink;

    let root = tempfile::tempdir().unwrap();
    let target = root.path().join("target");
    fs::create_dir(&target).unwrap();
    fs::write(target.join("flake.nix"), "{}").unwrap();
    let link = root.path().join("link");
    symlink(&target, &link).unwrap();

    assert_eq!(
      resolve_flake_directory(&link).map_err(|_| ()),
      Ok(fs::canonicalize(target).unwrap())
    );
  }

  #[cfg(unix)]
  #[test]
  fn resolve_flake_directory_resolves_symlinked_flake() {
    use std::os::unix::fs::symlink;

    let root = tempfile::tempdir().unwrap();
    let target = root.path().join("target");
    fs::create_dir(&target).unwrap();
    let flake = target.join("flake.nix");
    fs::write(&flake, "{}").unwrap();
    let source = root.path().join("source");
    fs::create_dir(&source).unwrap();
    symlink(&flake, source.join("flake.nix")).unwrap();

    assert_eq!(
      resolve_flake_directory(&source).map_err(|_| ()),
      Ok(fs::canonicalize(target).unwrap())
    );
  }
